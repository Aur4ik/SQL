...
--     CASE 
--         WHEN s.id IS NULL THEN CONCAT('ticket_id=', t.id)
--         WHEN t.id IS NULL THEN CONCAT('session_id=', s.id)
--     END AS info
...
-- SELECT v.full_name AS viewers_almaty, t.purchased_at AS last_tickets
-- FROM viewers v
-- LEFT JOIN tickets t ON t.viewer_id = v.id
-- WHERE v.city = 'Almaty'
-- GROUP BY t.id, v.id
...
-- GROUP BY m.id, m.title
-- HAVING AVG(r.rating) >= 8 AND COUNT(r.rating) >= 2
...
--     s.id AS session_id,
--     SUM(t.price_paid) AS revenue,
--     (SELECT AVG(sub.revenue)
--      FROM (
--          SELECT SUM(t2.price_paid) AS revenue
--          FROM sessions s2
...
-- 	WHEN t.price_paid > 2500 THEN 'premium'
-- END
-- FROM tickets t

--TASK 20
-- SELECT s.id AS session_id, h.capacity AS hall_capacity,
-- COUNT(t.id) AS paid_count,
--     CASE
...
--     CASE
--         WHEN COUNT(t.id) FILTER (WHERE t.status = 'paid') = 0 THEN 'new'
--         WHEN COUNT(t.id) FILTER (WHERE t.status = 'paid') BETWEEN 1 AND 2 THEN 'regular'
--         WHEN COUNT(t.id) FILTER (WHERE t.status = 'paid') BETWEEN 3 AND 5 THEN 'loyal'
--         ELSE 'vip'
--     END AS level
-- FROM viewers v
-- LEFT JOIN tickets t ON t.viewer_id = v.id
...
--     avg_rating,
--     CASE
--         WHEN avg_rating IS NULL THEN 'no_data'
--         WHEN avg_rating < 7 THEN 'risk'
--         WHEN avg_rating BETWEEN 7 AND 8.4 THEN 'ok'
--         ELSE 'hit'
--     END AS label
-- FROM movies;
-- task 23
...
--     END AS check_result
-- FROM tickets t
-- JOIN viewers v ON v.id = t.viewer_id
...
--         WHEN AVG(t.price_paid) = 3000 THEN 'expensive'
--     END AS class
-- FROM movies m
-- LEFT JOIN sessions s ON s.movie_id = m.id
-- LEFT JOIN tickets t 
...
-- HAVING 
--     COUNT(t.id) > 0
--     AND COUNT(t.id) FILTER (WHERE t.status = 'paid') = 0;
-- task 28
-- SELECT
--     v.id AS viewer_id,
--     v.full_name,
...
-- FROM viewers v
-- LEFT JOIN reviews r ON r.viewer_id = v.id
-- GROUP BY v.id, v.full_name;
-- task 29
-- SELECT
--     v.id AS viewer_id,
...
--     title,
--     paid_count
-- FROM city_movie_counts cmc
-- WHERE paid_count = (
--     SELECT MAX(paid_count)
--     FROM city_movie_counts sub
--     WHERE sub.city = cmc.city
-- );
